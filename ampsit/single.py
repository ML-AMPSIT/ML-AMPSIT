...
def create_predict_and_plot_global(
    meth, N, var, hpoint, vpoint, hour, tun, n_sample, figsize=(10, 6)
):
    # queste “global” sono già dichiarate in cima al file
  global importance_list, first_ord, score_list, pvalue_list, mse_list, mae_list, ypred, ytest
...
#############################################################################################
    ...
    if meth in [4,5]:
      ...
      ln1 =axs[0,1].plot(df.index, df['score'], color=metric_colors[0], label='Score')
      ln2 =axs[0,1].plot(df.index, df['mse'], color=metric_colors[1], label='MSE')
      ln3 =axs[0,1].plot(df.index, df['mae'], color=metric_colors[2], label='MAE')
      twin=axs[0,1].twinx()
